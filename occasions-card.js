class OccasionsCard extends HTMLElement {
    set hass(hass) {
        ...
        const numberOfDays = this.config.numberofdays || 365; //TODO: Implement property numberofdays
         ...
         occasions.forEach(occasion => {
             ...
             const occasionYear = occasionDate.getFullYear();
             ...
             occasion.ts = new Date(current.getFullYear() + yearToAdd, occasionMonth, occasionDay).getTime();
             occasion.diff = Math.round(Math.abs((currentDayTS - occasion.ts) / oneDay));
             if (occasion.diff > numberOfDays) occasion.ts = 0;
         });
         ...
         this.content.innerHTML = `
             ...
             ${occasionsToday || upcomingOccasions ? 
                 ...
                 `<div class='bd-none'>Inga födelsedagar dom kommande ${numberOfDays} dagar</div>`
             }
         `;
    }
    ...
    generateOccasionHtml(occasionList, todayText, isToday) {
        return occasionList.map(occasion => `
            <div class='bd-wrapper ${isToday ? 'bd-today' : ''}'>
                ...
                <div class='bd-when'>${isToday ? todayText : `om ${occasion.diff} dagar`}</div>
            </div>
        `).join("");
    }
